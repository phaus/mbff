...
class db {
 ...
 function query($query_string,$limit=0,$offset=0) {
  ...
  if(_debug){
  	global $_debug;
  	$_debug->add("<b>".$this->database.":</b><i>".getenv("SCRIPT_NAME")."|".__LINE__."</i>: ".$query_string."\n");
  }
...
$errormsg="<br />mysql error:</b> $this->errdesc\n";
$errormsg.="<br /><b>Query:</b> ".$this->queries[sizeof($this->queries)-1]."\n<br />";
$errormsg.="<b>Date:</b> ".date("d.m.Y @ H:i")."\n<br />";
$errormsg.="<b>Script:</b> ".getenv("REQUEST_URI")."\n<br />";
$errormsg.="<b>Referer:</b> ".getenv("HTTP_REFERER")."\n<br><br />";
$errormsg="<b>Database: $this->appname:</b> $errormsg\n<br />";
 
  if($this->show_error) $errormsg = "$errormsg";
  else $errormsg = "\n<!-- $errormsg -->\n";
  if(_debug){
  	global $_debug;
  	$_debug->add("<span style=\"background:red; color: #ffffff; font-face: bold\">ERROR: ".$this->queries[sizeof($this->queries)-1]."</span>");
  }
  die("</table><font face=\"Verdana\" size=2><b>SQL-DATABASE ERROR</b><hr />".$errormsg."</font>");
 }
}
?>